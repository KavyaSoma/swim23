  ...
  You can scroll down to see previous entries and to add participants to a heat click on "add participants" link.
      <div class="board-inner instructor_tabs">
        <center><ul class="nav nav-tabs nav_info" id="myTab">
            <div class="liner"></div>
              <li>
                <a href="{{url('heatsetup/'.$event_id)}}"  class="tab-one"title="Stage Summary">
                 ...
                 <li>
                  ...
                  <div class="tab-content tab_details">
                    ...
                    <div class="tab-pane fade in active" id="stagesummary">
                      ...
                      <form class="form-horizontal" style="background:#fff;" method="post" action="{{url('semiheatsetup/'.$event_id.'/'.$subevent_id)}}">
                        {{csrf_field()}}
...
<input type="hidden" class="form-control" name="id" value="{{ $events[0]->EventId }}" required>
<input type="hidden" class="form-control" name="childheatid" value="-1" required>
<input type="hidden" class="form-control" name="stagenumber" value="2" required>
</div>
</div> 
<div class="form-group">
...
<div class="col-sm-6">
<input type="text" class="form-control"  name="max_participants" value="{{ $events[0]->MaxParticipants }}" required> 
</div>
</div>
<div class="form-group">
...
@endif
      ...
      <div class="form-group">
        ...
        <div class="col-sm-6">
          <input type="text" class="form-control" value="{{ $events[0]->Relay }}" name="relay" readonly required> 
        </div>
      </div>
      <div class="form-group">
...
console.log('{{ url('oldsemifinal/'.$event_id.'/'.$subevent_id) }}');
$.ajax({
    url: '{{ url('oldsemifinal/'.$event_id.'/'.$subevent_id)  }}',
