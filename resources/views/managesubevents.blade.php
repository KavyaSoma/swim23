  ...
  <li class="breadcrumb-item active">Heat setup & Result entry</li>
   ...
   <div class="row text-center"> 
    ...
    @foreach($events as $event)
            <tr><td>{{ $event->SubEventName }}</td><td><a href="{{ url('heatsetup/'.$event->EventId.'/'.$event->SubEventId)}}" style="color:#000;">setup heats?</a></td><td><a href="{{ url('heatsetup/'.$event->EventId.'/'.$event->SubEventId)}}" style="color:#000;">results entry?</a></td></tr>
    @endforeach    
    </table>
      @if(count($events)>0)
