...
<div class="tab-pane fade in active" id="address">
    <form class="form-horizontal" style="background:#fff;padding:35px" method="post" action="{{url('/edit-clubaddress/'.$club_id)}}">
      {{csrf_field()}}
      @foreach($clubaddress as $club)
        <div class="col-sm-12">
            <div class="form-group">
              <label class="control-label col-sm-4" for="txt">Address:</label>
            <div class="col-sm-6">
              <input type="hidden" name="address_id" value="{{$address}}">
             <input type="text" class="form-control" id="txt" name="club_address" value="{{ $club->AddressLine1}}" required>
           </div>
           </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label class="control-label col-sm-4" for="txt">Post code:</label>
                  <div class="col-sm-6">
                      <input type="text" class="form-control" id="post-code" name="post_code" value="{{$club->PostCode}}" pattern="([0-9]){4,9}" required>
            ...
            <center>
               ...
               <a href="{{url('/edit-clubcontact/'.$club_id)}}" class="btn btn-primary">Next</a><br><br>
             </div>
            @endforeach
          </form>
        </div>
